...
export class ChatBus extends EventEmitter {
  ...
  private async _handleBroadcastPayload(payload: any) {
    ...
    if (payload.type === "message") {
      ...
      await this._onAck(payload as BroadcastAckPayload);
    }
  }
  ...
  private async _onPostgrestChangeMessage(p: PostgrestChangesMessagePayload) {
    ...
    if (p.sender_role === "agent") {
      ...
      try {
        ...
        Supabase.from("last_message_status")
          .update({
            user_last_message_time: new Date().toISOString(),
            user_last_message_status:
              this.activeCId === p.conversation_id ? "read" : "received",
          })
          .eq("conversation_id", p.conversation_id); // update last message status for both users
      } catch (e) {
  ...
  private async _onAck(p: BroadcastAckPayload) {
  ...
  async syncConversationFromServer(conversation_id: string) {
    ...
    const { data: d, error: e } = await Supabase.from("last_message_status")
      .update({
        user_last_message_time: new Date().toISOString(),
        user_last_message_status:
    ...
    if (error?.message || e?.message) {
      console.warn(
        "syncConversationFromServer error",
    ...
    if (data && data.length > 0) {
      localChannel = Supabase.channel(`inbox:agent:${data[0].sender_id}`, {
        config: { broadcast: { ack: true } },
      });
    }
    for (const m of data ?? []) {
  ...
  async syncAll() {
    ...
    for (const c of convs ?? []) {
      convList.push(c.id);
      await this.syncConversationFromServer(c.id);
    }
